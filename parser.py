...
Lenta = db["Lenta"]
Catalog = db["Catalog"]
ShopsId = db["ShopsName"].find_one({"title": "Лента"})["_id"]
        ...
        while (totalCount > offsetLast):
            ...
            for v in r.json()["skus"]:
                ...
                if title:
                    ...
                    symbol = re.search("[млгрМЛГРКГ]{1,}", title[0])[0].lower()
                ...
                d = Catalog.find_one({"title": {"$regex": find}})
                #ищем твоар сверху и если находим проверяем тот ли это товар по категории и массе и если нет создаем новый
                if (d == None) or (d['weight'] != decimal) or (d['category'] != v["gaCategory"]):
                    print("Добавляем товар")
                    d = Catalog.insert_one({
                        "title": v["title"],
                ...
                di = Lenta.find_one_and_update({"title": v["title"]}, {"$set": {
                    "value": v["cardPrice"]["value"],
                    "idProduct": d.inserted_id or d["_id"],
                    ...
                    "productUrt": "http://lenta.com" + v["skuUrl"],
                    "availabilityShop": getAvailability("http://lenta.com" + v["skuUrl"]),
                    # "category": v["gaCategory"]
                }}, upsert=True, return_document=ReturnDocument.AFTER)

                Catalog.update_one({"_id": ObjectId(d.inserted_id)}, {
                                       "$addToSet": {"shops": {"ShopsId": ShopsId, "tovarId": di["_id"]}}})
